...
**II Саммит талантов · трек ИИ · Иван Эйдлин**

<a href="https://colab.research.google.com/github/ozmld/summit-ai-track/blob/main/site/public/notebooks/d2p2_student.ipynb" target="_parent"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/></a>

---

Мы с нуля обучим модель, которая предсказывает: **сдаст ли школьник ОГЭ по математике** — по его оценкам, посещаемости и активности на Учи.ру.
...
Ноутбук идёт вместе с моим рассказом. Там где `# TODO` — дописываете сами. Кто застрял — поднимает руку, я подхожу.
...
def task_block(
    ...
    *,
    ...
    Параметр minutes оставлен в сигнатуре для совместимости с конспектом,
    но в сам ноутбук не попадает — учитель видит только название задания.
    """
    _ = minutes
    header = f"""## Задание {number} · {title}
...
Импортируем всё **сразу** — так принято в реальных проектах. Дальше в заданиях
дополнительные импорты не потребуются.
""",
        student_code=f"""import pandas as pd
import numpy as np
...
# датасет лежит на сайте саммита
URL = "{DATA_URL}"

# TODO: прочитайте CSV по ссылке через pd.read_csv(URL)
df = # _____
...
""",
        student_code="""X = df.drop(columns=['passed'])
y = df['passed']
...
""",
        filled_code="""X = df.drop(columns=['passed'])
y = df['passed']
...
""",
        student_code="""# KNN чувствителен к масштабу признаков, поэтому оборачиваем в Pipeline со StandardScaler.
...
""",
        student_code="""y_pred = model.predict(X_test)   # берём предсказания нашей LogReg
...
# _____
""",
        filled_code="""y_pred = model.predict(X_test)
...
""",
        student_code="""depths = range(1, 21)
train_acc, test_acc = [], []
